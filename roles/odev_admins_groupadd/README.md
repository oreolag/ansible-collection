...
Create the `odev-admins` group and add existing users from the cluster CMDB variables.
...
```yaml
---
- name: Configure odev-admins group
  hosts: managed_hosts
  become: true
  roles:
    - oreol.mgmt.odev_admins_groupadd
```

Or invoke the collection playbook with your inventory:

```bash
ansible-playbook oreol.mgmt.odev_admins_groupadd -i hosts -e oreol_target=managed_hosts
```
